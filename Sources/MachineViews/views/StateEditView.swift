        ...
        GeometryReader { reader in
            ScrollView {
                VStack(alignment: .leading) {
                    LineView(root: viewModel.$machine.asBinding, path: viewModel.path.name, label: viewModel.name)
                    ...
                    ForEach(Array(viewModel.actions.enumerated()), id: \.0) { (index, action) in
                        CodeView(root: viewModel.$machine.asBinding, path: viewModel.path.actions[index].implementation, language: .swift) { () -> AnyView in
